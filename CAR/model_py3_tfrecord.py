...
class denoiser(object):
    def __init__(self, sess, input_c_dim=3, batch_size=64, patch_size=160):
        ...
        self.Y = CIR_CAR(self.X, is_training=self.is_training)
    ...
    def test(self, test_files_gt, test_files_bl, ckpt_dir, save_dir):
        ...
        for idx in range(len(test_files_gt)):
            ...
            print("%s, JPEG PSNR: %.2fdB, Final PSNR: %.2fdB, MS-SSIM: %.5f, Time: %.4fs" % (imagename, psnr_bilinear, psnr, ssim, test_time))
            psnr_sum += psnr
        ...
        print("--- Test --- Average PSNR JPEG: %.2fdB, Final: %.2fdB, MS-SSIM: %.5f, Running Time: %.4fs ---" % (avg_psnr_initial, avg_psnr, avg_ssim, test_sum))
